...
fn scoring_shape_accepts_general_research_findings_and_plans() {
    ...
    let concrete_controls = normalize_for_compare(
        "Before enabling a high-risk tool, require concrete controls: scope boundaries, \
         approval safeguards, secret handling, and immutable audit logs.",
    );
    assert!(has_tradeoff_or_structure(&concrete_controls));
