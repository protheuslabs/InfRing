...
Claude Code is available as an opt-in live probe through the local `claude` CLI.
It is not included in the default system list because it can use real account
usage. The probe is model-controlled: it passes the harness `--model` directly
to `claude --model` and does not silently route to `sonnet` or another stronger
model. If `INFRING_CLAUDE_CODE_MODEL` is set, it must equal the requested
harness model or the run is reported as `claude_code_control_model_mismatch`.
