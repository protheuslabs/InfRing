...
// Layer ownership: adapters/runtime::agent-engines::context-store.
//
...
function sourceKindForRole(role, explicit) {
  ...
  if (kind) return kind;
  if (role === 'tool') return 'tool_result_bundle';
  if (role === 'system') return 'status_summary';
  return 'interaction_unit';
}
...
function appendAtomToState(state, row) {
  ...
  const candidate = {
    ...
    source_kind: sourceKindForRole(role, row.source_kind),
...
function materializeFromState(state) {
  const topology = buildTopology(state.session_id, state.atoms);
