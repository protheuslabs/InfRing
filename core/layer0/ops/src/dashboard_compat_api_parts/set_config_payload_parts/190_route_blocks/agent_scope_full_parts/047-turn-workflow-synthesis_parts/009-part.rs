...
fn workflow_final_synthesis_retry_prompt_context(
    ...
    clean_text(
        &format!(
            "Internal final-response verifier retry. The previous candidate failed `{}`. Previous excerpt: {}.{} Produce the user-facing answer from the same recorded evidence and user goal. The visible answer must still stand on its own and be useful if formatting is stripped away; any presentation shape is fine if it fits the query, but finish the structure you start. Lead with the best bounded answer the evidence supports, then state limits or gaps. If the failure names missing coverage lanes, cover each named lane or explicitly mark its evidence as weak or missing. If the failure names missing citation/source signal, preserve compact source grounding for claims supported by recorded evidence, using whatever natural wording fits the answer. If the failure names answer_units_not_traceable_to_evidence, keep concrete claim wording only when the same substance is mirrored by recorded relevant_extract, support_snippet, evidence_claims, or primary answer units; do not promote raw claim_hints or source-title paraphrases into factual prose unless that substance also appears in recorded extract text. When recorded answer-ready units or evidence claims exist, restate the strongest supported one before naming gaps. Do not say that the evidence lacks concrete findings unless the recorded answer-unit and evidence-claim material is actually empty or purely gap-oriented. Remove unsupported category labels, exact dates, ages, numbers, or other modifiers unless they are present in the recorded evidence, or clearly mark them as inference. Do not expand a supported category, model, option, entity, or risk label into unsourced subexamples or details; state the label and evidence gap instead, unless the extra detail is clearly labeled as inference. If the failure names retrieval recap or the previous candidate opened by reporting tool/search/retrieval status, convert EvidencePacket answer-ready extract/claim material into answer units instead of listing sources or tool status. Do not mention this verifier, workflow gates, tool traces, internal outcome posture, or a required output format.",
