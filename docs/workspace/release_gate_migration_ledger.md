...
| `agent_runtime_live_gateway_socket_advisory` | Live Gateway Agent Runtime socket proof is optional release evidence. The current-host probe is manual/live-only; the disposable-host probe is release-safe when a lane can start a temporary Gateway. | For operator inspection, start or restart Gateway, wait for `infring gateway status --json` to report healthy, then run `npm run -s ops:agent-runtime:socket-live-gateway:guard`. For release-safe evidence, run `npm run -s ops:agent-runtime:socket-disposable-gateway:guard` and attach `core/local/artifacts/agent_runtime_socket_disposable_gateway_guard_current.json` when available. | If the live probe fails in an emergency release, keep the deterministic socket guards as blocking evidence, omit the optional live artifact, and open a follow-up issue before publishing the next release candidate. |
